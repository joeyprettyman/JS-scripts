$("#input-keyword").keydown(function() {

    var style = document.createElement('style');
    style.type = 'text/css';
    style.innerHTML =
        '.typeahead-list-item { color: #878787 !important; }' +
        '.typeahead-list .typeahead-list-item:focus, .typeahead-list .typeahead-list-item:hover { color: #878787 !important; background: #ececec !important; }' +
        '.linkText { color: #eeb301; }' +
        '.typeahead-list-item:hover { color: #000000 !important; }';
    document.getElementsByTagName('head')[0].appendChild(style);

    setTimeout(function() {

        // What did we search for?
        var mySearch = $("#input-keyword").val();
        var mySearch = mySearch.charAt(0).toUpperCase() + mySearch.slice(1);

        var myResults = $(".typeahead-list-item");

        // Compare keyword against the following...
        var myDictionary = [
            'Litigation',
            'Litigations',
            'Litigate',
...
var style = document.createElement('style');
style.type = 'text/css';
style.innerHTML = '.search-block.search-section.print--hide { z-index: 110 !important; }' +
    '.search-section-row.form-group.jumbo-input.search-field { z-index: 110 !important; }' +
    '.blockout { background: black; width: 100%; height: 100%; position: fixed; top: 0; left: 0; display: none; opacity: 0.2; z-index: 109 !important; display: block; }' +
    '.typeahead-list.typeahead-list--undefined.ng-scope { padding-bottom: 25px; }' +
    '.loading-overlay { display: none !important; }' +
    '.link-block { min-width: 0px !important; }';

document.getElementsByTagName('head')[0].appendChild(style);

$("input#input-keyword").focus(function() {

    if ($("div.blockout").val() == undefined) {

        $("div.search-block.search-section.print--hide").append("<div class='blockout' style='display: none;'></div>");

        $("div.blockout").fadeIn(250);

        /*$( "div.search-block.search-section.print--hide" ).append( "<div class='blockout'></div>" );*/
        $("div.typeahead-list.typeahead-list--undefined.ng-scope").slideDown();
        //.css ( "display", "block" );
    }

});

$("input#input-keyword").blur(function() {

    if ($("div.blockout").val() !== undefined) {
        //$( "div.blockout" ).fadeToggle( 300 );

        $("div.blockout").fadeOut(400, function() {
            $("div.search-block.search-section.print--hide")[0].children[4].remove();
        });
        $("div.typeahead-list.typeahead-list--undefined.ng-scope").slideUp(190);
...
function buildServicesSection() {
    ...
    /*// Every Other (Odd 1, 3, 5, etc.)
    console.log(
      containerForHTML2.filter((x, i) => i % 2 )
    )
    
    // Every (Event 0, 2, 4, etc.)
    console.log(
    
      //containerForHTML2.filter((x, i) => i / 2 == 1 )
      containerForHTML2.filter((x, i) => i % 2 )
    )*/
